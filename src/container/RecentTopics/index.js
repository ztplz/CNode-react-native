...
class RecentTopics extends Component {
  ...
  renderHeader() {
    ...
    if(this.props.navigation.state.params.authorname === '我') {
      if(this.props.dataOfMe.recent_topics.length === 0) {
        return (
          <View style={styles.flatlistHeader}>
            <Text style={[styles.flatlistHeaderText, { color: this.props.screenProps.isNightMode? NIGHT_RECENT_FLATLIST_HEADER : null }]}>你最近未发布主题</Text>
          </View>
        )
      }
      return null;
    ...
    if(this.props.dataOfUser.recent_topics.length === 0) {
      return (
        <View style={styles.flatlistHeader}>
          <Text style={[styles.flatlistHeaderText, { color: this.props.screenProps.isNightMode? NIGHT_RECENT_FLATLIST_HEADER : null } ]}>他最近未发布主题</Text>
        </View>
      )
    }
  ...
  render() {
    const { isRefreshing, error, dataOfMe, screenProps, dataOfUser, actions, navigation } = this.props;
    return (
      <View style={[styles.container, { backgroundColor: screenProps.isNightMode? NIGHT_BACKGROUND_COLOR : null }]}>
        <FlatList
          data={navigation.state.params.authorname === '我'? dataOfMe.recent_topics : dataOfUser.recent_topics}
